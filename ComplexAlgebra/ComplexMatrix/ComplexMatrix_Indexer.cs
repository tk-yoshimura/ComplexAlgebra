...
namespace ComplexAlgebra {
    public partial class ComplexMatrix {
        public Complex this[int row_index, int column_index] {
            get => e[row_index, column_index];
            set => e[row_index, column_index] = value;
        }

        public Complex this[Index row_index, Index column_index] {
        ...
        public ComplexVector this[Range row_range, Index column_index] {
            get {
                int c = column_index.GetOffset(Columns);
                (int ri, int rn) = row_range.GetOffsetAndLength(Rows);

                Complex[] m = new Complex[rn];
                for (int i = 0; i < rn; i++) {
                    m[i] = e[i + ri, c];
                }
        ...
        public ComplexVector this[Index row_index, Range column_range] {
            get {
                int r = row_index.GetOffset(Rows);
                (int ci, int cn) = column_range.GetOffsetAndLength(Columns);

                Complex[] m = new Complex[cn];
                for (int j = 0; j < cn; j++) {
                    m[j] = e[r, j + ci];
                }
        ...
        public ComplexMatrix this[int[] row_indexes, int[] column_indexes] {
            get {
                Complex[,] m = new Complex[row_indexes.Length, column_indexes.Length];
        ...
        public ComplexMatrix this[IEnumerable<int> row_indexes, IEnumerable<int> column_indexes] {
            get => this[row_indexes.ToArray(), column_indexes.ToArray()];
            set => this[row_indexes.ToArray(), column_indexes.ToArray()] = value;
        }

        public ComplexMatrix this[int[] row_indexes, Range column_range] {
            get {
                (int ci, int cn) = column_range.GetOffsetAndLength(Columns);

                Complex[,] m = new Complex[row_indexes.Length, cn];
                for (int i = 0; i < row_indexes.Length; i++) {
                    for (int j = 0; j < cn; j++) {
        ...
        public ComplexMatrix this[IEnumerable<int> row_indexes, Range column_range] {
            get => this[row_indexes.ToArray(), column_range];
            set => this[row_indexes.ToArray(), column_range] = value;
        }

        public ComplexMatrix this[Range row_range, int[] column_indexes] {
            get {
                (int ri, int rn) = row_range.GetOffsetAndLength(Rows);

                Complex[,] m = new Complex[rn, column_indexes.Length];
                for (int i = 0; i < rn; i++) {
                    for (int j = 0; j < column_indexes.Length; j++) {
        ...
        public ComplexMatrix this[Range row_range, IEnumerable<int> column_indexes] {
